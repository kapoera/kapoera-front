...
import { Grid, Image, Label, Progress, Segment, Radio, Form, Button, Input } from 'semantic-ui-react';
import styled from 'styled-components';
import { GameCardProps, University, GameStatus, Winner } from '@/components/GameCard';
import EventList from '@/components/EventList';
import GameBettingEnd from '@/components/GameBettingEnd';
...
const AdminGame: React.FC = () => {
    ...
    const [{ starting_time, game_type }, setGameData] = useState(
        defaultState
    );
    ...
    const changeStatus = async () => {
        const {
            ...
            '/api/private/admin/playing-type',
            { playing: playing, game_type: gameId }
        );
        if (data.success) {
            console.log(data.success)
        }
    ...
    const changeScore = async () => {
        const {
            ...
            { result: result, game_type: gameId }
        );
        if (data.success) {
            console.log(data.success)
        }
    ...
    return (
        <GameContainer>
            ...
            <Segment>
                <div style={{ marginLeft: '20px', display: 'flex' }}>
                    ...
                    <Form>
                        ...
                        >
                            <Input type='number' max={5} min={0} value={result[University.Kaist]} onChange={handleKaistScore} />
                            <Input type='number' max={5} min={0} value={result[University.Postech]} onChange={handlePostechScore} />
                        </Form.Field>
            ...
            {/* <EventList isAdmin={true} gameId={gameId}></EventList> */}
        </GameContainer>
    )
}
